...
__all__ = ['LeNetCascade']


class LeNetCascade(nn.Module):
    def __init__(self, num_classes=10, backbone_fc=True):
        super(LeNetCascade, self).__init__()
    ...
    def forward(self, x):
        ...
        out = torch.cat([extractor1, extractor2, out],dim=2)  # [b, c, 3, 1]
...
def demo():
    net = LeNetCascade(num_classes=10, backbone_fc=False)
