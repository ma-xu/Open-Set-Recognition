...
class ModulatedAttLayer(nn.Module):
    ...
    def __init__(self, in_channels, reduction=2, mode='embedded_gaussian', height=7, width=7):
        ...
        self.fc_spatial = nn.Linear(self.height * self.width * self.in_channels, self.height * self.width)
    ...
    def embedded_gaussian(self, x):
        ...
        x_flatten = x.view(-1, self.height * self.width * self.in_channels)

        spatial_att = self.fc_spatial(x_flatten)
        spatial_att = spatial_att.softmax(dim=1)

        spatial_att = spatial_att.view(-1, self.height, self.width).unsqueeze(1)
...
def demo():
    x = torch.rand(2,4,5,4)
    net = ModulatedAttLayer(4,height=5,width=4)
    a,b = net(x)
    print(a.shape)
    print(len(b))
