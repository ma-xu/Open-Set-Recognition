...
class DFPNet(nn.Module):
    def __init__(self, backbone='ResNet18', num_classes=1000,
        ...
        self.centroids = nn.Parameter(torch.randn(num_classes + 1, self.classify_dim))
        self.classifier = nn.Sequential(
            nn.ReLU(inplace=True),
            nn.Linear(self.feat_dim, self.classify_dim)
        )
    ...
    def forward(self, x):
        ...
        classify_fea = self.classifier(embed_fea)
