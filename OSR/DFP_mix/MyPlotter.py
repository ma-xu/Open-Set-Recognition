...
def plot_distance(net,
    ...
    for i in range(args.train_class_num):
        ...
        cls_dist = cls_dist[0, -(args.tail_number)]  #remove the tail examples.
        # cls_dist = cls_dist / (max(cls_dist))  # normalized to 0-1, we consider min as 0.
        # min_distance = min(cls_dist)
        min_distance = 0
        max_distance = max(cls_dist)
        hist = torch.histc(torch.tensor(cls_dist.clone().detach()), bins=args.bins, min=min_distance, max=max_distance)
        results[i]['hist']=hist
